...
export async function adminSend<T, B extends object = object>(method: string, path: string, body: B): Promise<T>
export async function adminSend<T>(method: string, path: string): Promise<T>
export async function adminSend<T>(method: string, path: string, body?: object): Promise<T> {
  const response = await fetch(base + path, { method, headers: { 'Content-Type': 'application/json', Accept: 'application/json' }, body: body === undefined ? undefined : JSON.stringify(body) }); const text = await response.text(); let json: AdminResponse<T> | null = null
  try { json = text ? JSON.parse(text) as AdminResponse<T> : null } catch { throw new Error(`${path} returned invalid JSON`) }
  if (!response.ok || !json || json.code !== 0) throw new Error(json?.message || `${path} failed`); return json.data
}
