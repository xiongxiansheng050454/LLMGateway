...
- `server/internal/proxy/`：`proxy.go`（编排依赖装配与代理错误）、`contracts.go`（协议中立请求/响应/usage contract）、`authentication.go`、`routing.go`、`billing.go`、`ratelimiting.go`、`orchestration.go`（按代理业务职责命名，不以 OpenAI 下游协议命名）
